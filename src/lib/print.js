...
const { isShownRecently, markShown, projectHasFundOSSConfigInPackage } = require('./limit')
...
function run() {
    ...
    // Only run collect if project has config in package
    if (projectHasFundOSSConfigInPackage()) {
        collect.run()
        return // We dont need to show reminder message any more
    }
